...
def chaifen(file,sheeti,colu,path):#拆分表格
    ...
    if ret ==1 :
        ...
        for i in list(c.keys()):
            ...
            if path != '':
                f.save(path + "/" + i.strip('\t') + '.xls')
            else:
                f.save(i.strip('\t') + '.xls')
        print('已生成文件：', b)
        root1 = Tk(className='处理结果')
        textLabel = Label(root1, justify=LEFT,width=50)
        var1 = "文件名:<< % s>>" % e1.get()
        var2 = "第几张表:<< % s>>" % e2.get()
        ...
        j=len(bbb)/25
        t = Text(root1, height=j,width=50)
        t.insert('insert', bbb)
        t.pack()
        theButton = Button(root1, text="确定", width=10, command=root1.quit)
        theButton.pack(padx=10, pady=5)
    else:
        root1 = Tk(className='未执行拆分！！')
